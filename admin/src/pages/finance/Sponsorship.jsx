...
export default function Sponsorship() {
  ...
  const fetchItems = useCallback(async () => {
    ...
    try {
      ...
      const res = await sponsorshipAPI.getAll(params);
      const data = res.data;
      setItems(data.data || []);
  ...
  const handleStatusChange = async (id, newStatus) => {
    ...
    try {
      await sponsorshipAPI.updateStatus(id, newStatus);
  ...
  const handleDelete = async () => {
    setDeleting(true);
    try {
      await sponsorshipAPI.delete(deleteDialog.id);
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Manage incoming sponsorship requests"
      />
      ...
      <div className="bg-white rounded-xl border border-slate-100 shadow-sm overflow-hidden">
        {loading ? (
          ...
          <EmptyState
            message="No sponsorship inquiries"
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-slate-50">
                  ...
                  <th className="px-4 py-3 text-left text-xs font-semibold text-slate-500 uppercase tracking-wide">Contact Person</th>
              ...
              <tbody className="divide-y divide-slate-50">
                {items.map((item) => (
                  <tr key={item._id} className="hover:bg-slate-50 transition-colors">
                    <td className="px-4 py-3 font-medium text-slate-800 whitespace-nowrap">{item.organization}</td>
                    <td className="px-4 py-3 text-slate-600">{item.contactPerson || item.name}</td>
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        <button
                          onClick={() => navigate(`/sponsorship/${item._id}`)}
      ...
      <ConfirmDialog
        ...
        message="Are you sure you want to delete this sponsorship inquiry? This action cannot be undone."
        loading={deleting}
      />
    </div>
  );
