...
export const getCountryDial = (name) => COUNTRY_DATA.find((c) => c.name === name)?.dial || '';
export const getCountryISO = (name) => COUNTRY_DATA.find((c) => c.name === name)?.iso?.toLowerCase() || '';
export const getCountryFlag = (name) => {
  const d = COUNTRY_DATA.find((c) => c.name === name);
  return d ? isoToFlag(d.iso) : '';
};

export const COUNTRIES = COUNTRY_DATA.map((c) => c.name);
export const COUNTRY_OPTIONS = COUNTRY_DATA.map((c) => ({ value: c.name, label: c.name }));
