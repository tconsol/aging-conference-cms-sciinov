...
const CurvedInput = ({
  ...
  useLayoutEffect(() => {
    ...
    if (Math.abs(next - scrollRef.current) > 0.5) { scrollRef.current = next; setScrollLen(next); }
  ...
  const handleSubmit = e => { if (e?.preventDefault) e.preventDefault(); if (onSubmit) onSubmit(val); };
  ...
  const handleSurfaceClick = e => {
    ...
    if (svg && geom && layout && textEl && display.length) {
      try {
        ...
        idx = best;
      } catch { idx = display.length; }
    }
    input.focus();
    try { input.setSelectionRange(idx, idx); } catch { /* unavailable */ }
    setCaretIndex(idx);
  };
          ...
          <g transform={`translate(${round2(ix)} ${round2(iy)}) rotate(${round2(iconAngle)})`} aria-hidden="true">
            {icon || (
              <>
                ...
                <rect x={-ew / 2} y={-eh / 2} width={ew} height={eh} rx={1.4} fill="none" stroke="#ffffff" strokeWidth={sw} strokeLinejoin="round" />
                <path d={`M ${round2(-ew / 2)} ${round2(-eh / 2 + sw * 0.4)} L 0 ${round2(eh * 0.14)} L ${round2(ew / 2)} ${round2(-eh / 2 + sw * 0.4)}`} fill="none" stroke="#ffffff" strokeWidth={sw} strokeLinejoin="round" strokeLinecap="round" />
              </>
            )}
          </g>
        )}
